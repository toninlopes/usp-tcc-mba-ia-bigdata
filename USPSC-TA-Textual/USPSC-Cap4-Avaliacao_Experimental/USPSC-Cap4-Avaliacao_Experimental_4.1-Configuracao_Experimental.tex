...
O processo de \textit{fine-tuning} do BERTimbau, descrito em detalhes na \autoref{subsubsec:arquitetura_bertimbau}, foi executado com os hiperparâmetros consolidados no \autoref{tab:hiperparametros_bertimbau}.

\begin{table}[!htbp]
	\caption[Hiperparâmetros do fine-tuning do BERTimbau]{Hiperparâmetros utilizados no \textit{fine-tuning} do BERTimbau}
	\label{tab:hiperparametros_bertimbau}
	\centering
...
O \textit{pipeline} de avaliação foi executado em ambiente Python 3.11, com as principais dependências listadas no \autoref{tab:dependencias}.
