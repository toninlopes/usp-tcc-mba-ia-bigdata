...
\begin{enumerate}
	...
	\item \textbf{Comparação e análise}: as classificações individuais são comparadas entre as abordagens e analisadas à luz do contexto macroeconômico e financeiro do período, conforme apresentado no \autoref{Cap:Ava_Exp}.
\end{enumerate}
...
\begin{figure}[!htb]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tikzpicture}[
			...
			\node[box, right=of classif] (compar) {Comparação\\e Análise};
			
			\node[above=0.35cm of coleta, font=\scriptsize] {API X/Twitter};
			\node[above=0.35cm of compar,  font=\scriptsize] {Resultados por Abordagem};
			
			\draw[arrow] (coleta)   -- (db);
			\draw[arrow] (db)       -- (rotul);
			\draw[arrow] (rotul)    -- (preproc);
			\draw[arrow] (preproc)  -- (classif);
			\draw[arrow] (classif)  -- (compar);
...
\begin{table}[!htb]
	...
	\begin{tabular}{ccp{10cm}}
		...
		\hline
		\texttt{streamlit} & $\geq$ 1.35 & Ferramenta de anotação manual (\textit{Classifier App}) e painel de AED (\texttt{exploration.py}) \\
		
		\hline
		\texttt{pandas} & $\geq$ 2.0 & Manipulação tabular do corpus e apoio à Análise Exploratória de Dados (AED) \\
		...
		\hline
		\texttt{plotly} & $\geq$ 5.20 & Visualizações interativas da AED e dos resultados por abordagem \\
		\hline
	\end{tabular}
	\legend{Fonte: Elaborado pelo autor.}
\end{table}

O código-fonte do \textit{pipeline} está organizado como um pacote Python versionado, com dependências declaradas em arquivo \texttt{requirements.txt}, de modo a facilitar a reprodução dos experimentos em ambientes distintos. O repositório completo está disponível em: \url{https://github.com/toninlopes/x-sentimento-financeiro-bertimbau/tree/v1.0.0-tcc}.