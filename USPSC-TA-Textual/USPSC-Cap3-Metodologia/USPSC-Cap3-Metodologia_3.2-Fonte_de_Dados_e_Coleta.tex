...
Para cada veículo monitorado, as requisições foram parametrizadas conforme o \autoref{tab:params_api}. O intervalo temporal foi delimitado pelos campos \texttt{start\_time} e \texttt{end\_time}, ambos no formato ISO~8601 (\texttt{YYYY-MM-DDTHH:mm:ssZ}, UTC). O volume máximo de resultados por página foi fixado em 100 registros — limite superior permitido pelo \textit{endpoint} —, e a paginação foi controlada pelo parâmetro \texttt{pagination\_token}, utilizado de forma iterativa até o esgotamento dos resultados disponíveis.

\begin{table}[htb]
	\caption{Parâmetros utilizados nas requisições ao \textit{endpoint} \\ \texttt{GET /2/users/ \{id\}/tweets}}
	\label{tab:params_api}
...
O \textit{pipeline} de coleta é a primeira etapa de uma estrutura modular de seis estágios\footnote{O código-fonte completo do \textit{pipeline}, incluindo todos os módulos e arquivos referenciados ao longo deste trabalho, está disponível em: \url{https://github.com/toninlopes/x-sentimento-financeiro-bertimbau/tree/v1.0.0-tcc}. Para acessar diretamente um arquivo específico, basta concatenar esse endereço (substituindo \texttt{tree} por \texttt{blob}) ao caminho relativo do arquivo — por exemplo, a classe \texttt{TwitterAPIExtractor}, definida no arquivo \texttt{app/core/extraction/twitter\_api.py}, pode ser consultada em: \url{https://github.com/toninlopes/x-sentimento-financeiro-bertimbau/blob/v1.0.0-tcc/app/core/extraction/twitter_api.py}.}. Os módulos que compõem essa etapa, com suas responsabilidades bem delimitadas, são:

\begin{itemize}

	\item \texttt{app/core/extraction/twitter\_api.py}: a classe \texttt{TwitterAPIExtractor} herda da classe abstrata \texttt{BaseExtractor} e é responsável pela construção e execução das requisições HTTP à API via biblioteca \texttt{requests}. Recebe como parâmetros o identificador do usuário (\texttt{x\_user\_id}), o intervalo temporal e, opcionalmente, o token de paginação;
	...
	\item \texttt{app/shared/db/database.py}: módulo transversal que implementa a classe \texttt{Database Manager}, responsável pela interface de comunicação com o banco de dados PostgreSQL — incluindo as operações de consulta, inserção e atualização utilizadas por todos os estágios do \textit{pipeline};
...
\begin{table}[!htb]
	...
	\begin{tabular}{p{4.5cm}p{4.5cm}p{5.8cm}}
		...
		\hline
		\texttt{tweets} & \texttt{tweet\_id} & Identificador único da publicação na plataforma (\texttt{UNIQUE}) \\ & \texttt{username} & Identificador numérico do veículo autor \\
		...
		(JSONB) \\
		\hline
		\texttt{tweets\_classification} & \texttt{tweet\_id} & FK → \texttt{tweets.id} (\texttt{ON DELETE CASCADE}) \\ & \texttt{sentiment} & Polaridade atribuída: \textit{positivo}, \textit{negativo} ou \textit{neutro} \\ & \texttt{why\_sentiment} & Justificativa textual para a polaridade classificada \\ & \texttt{is\_finance\_news}   & Relevância financeira classificada: 1~=~financeiro, 0~=~não financeiro \\ & \texttt{why\_is\_finance\_news} & Justificativa textual para a classificação de relevância \\ & \texttt{classificator} & Identificador do classificador: \texttt{"Humano"}, \texttt{"FinBERT-PT-BR"}, \texttt{"BERTimbau"} ou outro modelo futuro \\
		\hline
		\texttt{collection\_log} & \texttt{search\_term} & Parâmetros da coleta agendada (JSONB) \\ & \texttt{tweets\_collected}    & Total de registros inseridos no 	ciclo \\
