...
\begin{itemize}
	\item \texttt{app/dashboard/app.py}: \textit{launcher} multi-página Streamlit, invocado via \texttt{streamlit run app/dashboard/app.py} (acessível em \texttt{http://localhost:8501}). Agrega em uma única interface três páginas: \textit{Classification} (\texttt{classifier.py}), \textit{Analytics} (\texttt{app/dashboard/pages/eda.py}) e \textit{Exploração dos Dados} (\texttt{app/dashboard/pages/exploration.py}), permitindo ao anotador alternar entre a tarefa de rotulagem e a inspeção dos dados sem trocar de aplicação;
	
	\item \texttt{app/dashboard/pages/annotation.py}: página principal de anotação, que implementa a interface de classificação tweet a tweet e obtém os dados diretamente do banco de dados via
	\texttt{app/shared/db/database.py}.
\end{itemize}
